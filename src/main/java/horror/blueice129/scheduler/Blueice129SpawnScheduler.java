 ...
 * Occurs every 15-30 minutes of playtime.
 */
public class Blueice129SpawnScheduler {
    private static final String TIMER_ID = "blueice129_spawn_timer";
    private static final Random RANDOM = Random.create();

    private static final int MIN_DELAY = 20 * 60 * 15; // 15 minutes
    private static final int MAX_DELAY = 20 * 60 * 30; // 30 minutes
    
    private static final int MIN_SPAWN_DISTANCE = 64;
    private static final int MAX_SPAWN_DISTANCE = 128;
    ...
    private static BlockPos findForestSpawnLocation(ServerWorld world, BlockPos playerPos) {
        ...
        for (int attempt = 0; attempt < 20; attempt++) {
            ...
            BlockPos surfacePos = world.getTopPosition(net.minecraft.world.Heightmap.Type.MOTION_BLOCKING_NO_LEAVES, testPos);
    ...
    private static boolean isSafeSpawnLocation(ServerWorld world, BlockPos pos) {
        ...
        if (world.getBlockState(pos).getFluidState().isStill() || 
            world.getBlockState(pos.down()).getFluidState().isStill()) {
            return false;
        }
     ...
     */
    public static boolean forceSpawn(MinecraftServer server, ServerPlayerEntity player) {
        ServerWorld world = server.getOverworld();
