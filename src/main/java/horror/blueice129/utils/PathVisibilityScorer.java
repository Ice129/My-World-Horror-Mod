...
public class PathVisibilityScorer {
    ...
    /**
     * Score a path by counting how many positions along it are visible to the
     * player.
     * Samples positions along a straight line from start to end and checks
     * visibility
     ...
     * @return Visibility score (lower = better for hiding). Returns -1 if player is
     *         null.
     */
    public static int scorePath(PlayerEntity player, World world, BlockPos start, BlockPos end, double maxDistance) {
        ...
        for (int i = 0; i <= sampleCount; i++) {
            ...
            // Find the actual ground level (skip non-collision blocks like grass, torches,
            // snow)
